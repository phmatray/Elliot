...
/// yields, roughly half to each. `Client.startReadLoop()` (`ACP/Client.swift`) is the only place in
/// this package allowed to iterate a live transport's `.messages`: if a second one ever iterated
/// the same instance, every other in-flight JSON-RPC request would silently time out, with a green
/// build and (absent this test) a green suite — a dropped reply looks identical to a slow one until
/// the timeout fires.
///
...
struct MessagesSingleConsumerTests {
    ...
    @Test("only Client.swift's read loop consumes a transport's messages")
    func onlyOneConsumer() throws {
        ...
        #expect(
            offenders.isEmpty,
            """
            \(offenders.joined(separator: ", ")) reads `.messages` outside Client.swift's read \
            loop. A second consumer of a live transport splits its AsyncStream between iterators \
            instead of each seeing every value — a dropped JSON-RPC reply, not a slow one. Route it \
            through Client, or add an entry to sanctionedConsumers naming the file and the reason it \
            is not reading a transport a Client is also reading (see #146 for what happens when a \
            mechanism like this one is quietly duplicated).
            """
        )
    }
}
