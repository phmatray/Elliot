...
public actor StdioTransport: Transport {
    ...
    public func launch(
        ...
        // Resolve symlinks
        let resolvedPath =
            (try? FileManager.default.destinationOfSymbolicLink(atPath: executablePath)) ?? executablePath
        let actualPath =
            resolvedPath.hasPrefix("/")
            ? resolvedPath
            : ((executablePath as NSString).deletingLastPathComponent as NSString).appendingPathComponent(
                resolvedPath)
